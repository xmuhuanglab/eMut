...
#' ActiveDriverCRE is a driver discovery tool for simple somatic mutations in cancer whole genomes
#'
...
#'
#'
ActiveDriverCRE = function(mutations,
                           elements,
                           sites = NULL,
                           window_size = 50000,
                           filter_hyper_MB = 30,
  ...
  mutated_results = do.call(rbind, parallel::mclapply(1:length(not_done), function(i) {
    ...
    result<-tryCatch({
      ...
      data.frame(id = not_done[i],
                 pp_element = NA, element_muts_obs = NA, element_muts_exp = -1, element_enriched = NA,
                 pp_site = NA, site_muts_obs = NA, site_muts_exp = NA, site_enriched = NA,
                 stringsAsFactors = FALSE)
    })
